...
# Build .icns from logo.png so the icon shows in Finder / Dock
if [ -f "logo.png" ]; then
  echo "▶ Building app icon (.icns)…"
  ICONSET="/tmp/DuoSound.iconset"
  rm -rf "$ICONSET" && mkdir "$ICONSET"
  for size in 16 32 64 128 256 512; do
    sips -z $size $size logo.png --out "${ICONSET}/icon_${size}x${size}.png"    2>/dev/null
    double=$((size * 2))
    sips -z $double $double logo.png --out "${ICONSET}/icon_${size}x${size}@2x.png" 2>/dev/null
  done
